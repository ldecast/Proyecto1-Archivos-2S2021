...
string getDotBlock(int _index_inode, int _start_inodes, int _start_blocks, string _path, int _index_dot)
{
...
string getDot_folder_block(int _start_blocks, int _index_block, string _path, int _index_dot)
{
    FILE *_file = fopen(_path.c_str(), "rb");
    CarpetasBlock cb;
    fseek(_file, _start_blocks, SEEK_SET);
    ...
    fread(&cb, 64, 1, _file);
    string dot =
        string(std::to_string(_index_block) + " [ fontsize=\"17\" label = <\n") +
